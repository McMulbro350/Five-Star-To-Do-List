/* ==========================================================================
   5 Star To-Do 2.0 — script.js (Firebase edition)
...
// Using a PERSISTENT, MULTI-TAB local cache (backed by IndexedDB) instead of
// the plain in-memory default. Two things this buys us:
//   1. Faster loads: on repeat visits, Firestore can show your last-known
//      lists immediately from the local cache while it reconnects to the
//      server in the background, instead of the screen staying blank/loading
//      until a fresh network round-trip finishes every single time.
//   2. Real multi-tab support: without "multiple tab manager", only ONE tab
//      in a browser gets to use the persistent cache at a time, and every
//      other tab silently falls back to memory-only (no offline support,
//      slower reconnects). This makes every tab a first-class citizen.
...
// Manually forces a fresh read straight from the Firestore SERVER, skipping
// the local cache entirely. The real-time listener above already keeps
// things in sync automatically, but after everything that came up with
// syncing across browsers, having a visible, deliberate "yes, I really did
// just check" button is worth the extra reassurance.
async function forceSyncNow() {
  ...
  try {
    ...
    const now = new Date();
    syncStatus.textContent = `Synced ✓ (${now.toLocaleTimeString()})`;
  } catch (err) {
    console.error("Manual sync failed:", err);
    const code = err && err.code ? err.code : "unknown-error";
    syncStatus.textContent = `Sync failed (error: ${code})`;
  } finally {
...
// Colors pulled from the app's own light blue palette (plus gold, matching
// the star ratings), so the confetti still feels like part of the same
// design instead of a generic effect.
const CONFETTI_COLORS = ["#2f86c9", "#4fb0e0", "#cde9fa", "#3893c2", "#f0a93b"];
...
newTaskForm.addEventListener("submit", (e) => {
  ...
  newTaskInput.value = "";
  newTaskInput.focus(); // ready for the next task immediately
});
